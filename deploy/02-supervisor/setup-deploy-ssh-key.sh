#!/usr/bin/env bash
# 在「应用机」上执行：为 DEPLOY_USER 生成 SSH 密钥并写入 authorized_keys
# 用法：
...
if ! id -u "${DEPLOY_USER}" >/dev/null 2>&1; then
  echo "用户不存在: ${DEPLOY_USER}（请先运行 install.sh 或手动创建）" >&2
  exit 1
fi
...
echo
echo "本机自测（私钥拷到客户端文件后）："
echo "  chmod 600 /tmp/${DEPLOY_USER}_key"
echo "  ssh -i /tmp/${DEPLOY_USER}_key -o BatchMode=yes -o StrictHostKeyChecking=accept-new ${DEPLOY_USER}@<应用机IP> 'echo ssh_ok'"
